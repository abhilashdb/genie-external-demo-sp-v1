...
# --------------------------------------------------------------------------
# Step (d) — Row-level security function + apply
# --------------------------------------------------------------------------
...
def apply_rls(w: WorkspaceClient, northstar: SPInfo, sunrise: SPInfo) -> None:
    log("Step d) Creating RLS function and applying to tables...")

    # NOTE: When a service principal authenticates to Unity Catalog,
    # current_user() returns the SP's applicationId (UUID) rather than a
    # human-readable display name. We match against the applicationId here.
    rls_sql = f"""
    CREATE OR REPLACE FUNCTION {FQ_SCHEMA}.dealership_rls(dealership_id STRING)
    RETURNS BOOLEAN
    RETURN
      CASE current_user()
        WHEN '{northstar.client_id}' THEN dealership_id = '{northstar.dealership_id}'
        WHEN '{sunrise.client_id}' THEN dealership_id = '{sunrise.dealership_id}'
        ELSE true
      END
    """.strip()
