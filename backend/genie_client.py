...
class GenieClient:
    ...
    async def _request(
        ...
        exponentially with jitter. Every event for this call is tagged with a
        short `request_id` so interleaved events (e.g. during stress tests) can
        be correlated back to a single logical request.
        """
        url = self._url(path)
        attempts = 0
        request_id = f"req-{secrets.token_hex(3)}"
        t0 = time.monotonic()
        ...
        while True:
            ...
            if attempts < _RATE_LIMIT_MAX_RETRIES:
                ...
                if sim_status:
                    ...
                    remaining = _RATE_LIMIT_MAX_RETRIES - attempts
                    await flow_events.publish(
                        self.session_id,
                        step="genie_rate_limit",
                        status="pending",
                        title=f"[{request_id}] [simulated] HTTP {sim_status} — backoff {delay:.1f}s (retry {attempts}/{_RATE_LIMIT_MAX_RETRIES})",
                        detail=(
                            f"simulated {sim_status}; exponential backoff ({_RATE_LIMIT_BASE_DELAY}s × 2^{attempts - 1}) ± 25% jitter, "
                            f"capped at {_RATE_LIMIT_MAX_DELAY}s. {remaining} retries left after this."
                        ),
                        payload={
                    ...
                    await flow_events.publish(
                        ...
                        step="genie_retry",
                        status="pending",
                        title=f"[{request_id}] [simulated] retrying {method} {path} (attempt {attempts + 1})",
                        detail=f"slept {delay:.1f}s, total wall time so far {_elapsed_ms()} ms",
                        payload={
                            "request_id": request_id,
                            "endpoint": path,
                            "method": method,
                            "attempt": attempts + 1,
                            "slept_seconds": round(delay, 2),
                            "elapsed_ms": _elapsed_ms(),
                            "simulated": True,
                        },
                    )
            ...
            try:
                ...
                await flow_events.publish(
                    ...
                    step="genie_call",
                    status="error",
                    title=f"[{request_id}] {method} {path} network error",
            ...
            if resp.status_code in _RATE_LIMIT_STATUSES and attempts < _RATE_LIMIT_MAX_RETRIES:
                ...
                delay = _compute_backoff(retry_after_hdr, attempts)
                body_snippet = (resp.text or "")[:240].strip()
                remaining = _RATE_LIMIT_MAX_RETRIES - attempts
                used_ra = bool(retry_after_hdr and _is_numeric(retry_after_hdr))
                source = (
                    f"Retry-After={retry_after_hdr}s"
                    if used_ra
                    else f"exponential {_RATE_LIMIT_BASE_DELAY}s × 2^{attempts - 1} ± 25% jitter (cap {_RATE_LIMIT_MAX_DELAY}s)"
                )
                await flow_events.publish(
                    self.session_id,
                    step="genie_rate_limit",
                    status="pending",
                    title=f"[{request_id}] HTTP {resp.status_code} — backoff {delay:.1f}s (retry {attempts}/{_RATE_LIMIT_MAX_RETRIES})",
                    detail=(
                        f"{method} {path} → {resp.status_code}. Backoff source: {source}. "
                        f"{remaining} retries left after this. "
                        + (f"Body: {body_snippet}" if body_snippet else "Body: <empty>")
                    ),
                    payload={
                        "request_id": request_id,
                        "endpoint": path,
                ...
                await flow_events.publish(
                    ...
                    step="genie_retry",
                    status="pending",
                    title=f"[{request_id}] retrying {method} {path} (attempt {attempts + 1})",
                    detail=f"slept {delay:.1f}s, total wall time so far {_elapsed_ms()} ms",
                    payload={
                        "request_id": request_id,
                        "endpoint": path,
            ...
            retry_suffix = f" after {attempts} retry" if attempts == 1 else (f" after {attempts} retries" if attempts else "")
            await flow_events.publish(
                self.session_id,
                step="genie_call",
                status=status_tag,
                title=f"[{request_id}] {method} {path} → {resp.status_code}{retry_suffix}",
                detail=f"total {_elapsed_ms()} ms" + (f", {attempts} rate-limit retries" if attempts else ""),
                payload={
                    "request_id": request_id,
                    "endpoint": path,
                    "method": method,
                    ...
                    "retries": attempts,
                    "elapsed_ms": _elapsed_ms(),
                    "response_body": (resp.text or "")[:240].strip() if status_tag == "error" else None,
                },
            )
            if status_tag == "error":
                raise GenieError(
    ...
    async def publish_sql(self, sql: str) -> None:
        ...
        await flow_events.publish(
            ...
            detail=(sql[:200] + ("…" if len(sql) > 200 else "")),
            payload={"sql": sql},
        )
